...
export interface IApiClient {
  sessions: {
    ...
    editPrompt(payload: RequestPayload<'session.editPrompt'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.editPrompt'>>>
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.editPrompt': sessionEditPromptValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    editPrompt: (payload, signal) => this.callUnary('session.editPrompt', payload, signal),
