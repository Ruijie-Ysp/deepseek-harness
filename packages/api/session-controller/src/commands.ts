...
export class SessionCommandController {
  ...
  async editPrompt(request: SessionEditPromptRequest): Promise<SessionEditPromptValue> {
    ...
    if (!nodes.includes(request.atSeq)) {
      reject(
        'edit-target-invalid',
    ...
    const target = agent.session.events[request.atSeq]
    ...
    const message: UserMessage = createUserMessage({
      ...
      source: {
        ...
        rpcId: request.requestId,
        ...(clientTimeZone === undefined ? {} : { clientTimeZone }),
        replacesSeq: request.atSeq,
      },
    })
    try {
      agent.followup(message)
    } catch (error) {
      reject('agent-busy', 'edit rejected', { reason: String(error) })
    }
    return { accepted: true }
  }
