...
export class SessionCommandController {
  ...
  /**
   * Rewrite one settled user message and queue the regenerated turn.
   * @param request - Session identity, the targeted surface seq, and replacement text.
   * @returns acknowledgement that the rewrite entered the Agent inbox.
   */
  async editPrompt(request: SessionEditPromptRequest): Promise<SessionEditPromptValue> {
    ...
    if (!Number.isSafeInteger(request.atSeq) || request.atSeq < 0
      || !nodes.includes(SessionSeq(request.atSeq))) {
      throw new RemoteError(
        'session/edit-target-invalid',
        `seq ${String(request.atSeq)} is not a current message on the session surface`,
        { atSeq: request.atSeq },
      )
    }
    const atSeq = SessionSeq(request.atSeq)
    const target = agent.session.snapshotEvents()[atSeq]
    ...
    if (!targetable) {
      throw new RemoteError(
        'session/edit-target-invalid',
        `seq ${String(request.atSeq)} does not project a human user message`,
        { atSeq: request.atSeq },
      )
    }
    ...
    const message: UserMessage = createUserMessage({
      content: [...request.content, ...preserved],
      source: {
        kind: 'user',
        rpcId: request.requestId,
        ...(clientTimeZone === undefined ? {} : { clientTimeZone }),
