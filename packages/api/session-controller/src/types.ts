...
declare module '@deepseek-ai/dsh-typert-protocol' {
  interface RemoteErrorDetailsMap {
    ...
    'session/edit-target-invalid': { readonly atSeq: number }
...
/** Request to rewrite one settled user message and regenerate from it. */
export interface SessionEditPromptRequest {
  /** Client-minted identity persisted on the rewrite message's user source. */
  readonly requestId: SessionRequestId
  readonly sessionId: SessionId
  /** Seq of the current surface node the rewrite replaces. */
  readonly atSeq: number
  /** Replacement text; the target's non-text blocks are preserved and reused. */
  readonly content: readonly { readonly type: 'text'; readonly text: string }[]
  readonly clientTimeZone?: string
}

/** Receipt after one rewrite enters the target Agent inbox. */
...
declare module '@deepseek-ai/dsh-llm' {
  interface MessageSourceMap {
     ...
     */
    'user-rpc': { kind: 'user'; rpcId: SessionRequestId; clientTimeZone?: string; replacesSeq?: number }
  }
}
