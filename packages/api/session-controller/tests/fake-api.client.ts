...
export class FakeApiClient {
  ...
  onEditPrompt: (payload: unknown) => Promise<RemoteResult<{ accepted: true }>> = () => Promise.resolve(ok({ accepted: true as const }))
  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      session: {
        ...
        editPrompt: payload => this.record('session.editPrompt', payload, this.onEditPrompt(payload)),
