...
import type { SessionEditPromptRequest, SessionPromptRequest, SessionRequestId } from '../src/types.ts'
import {
...
describe('subagent ownership fence', () => {
  ...
  it('rejects origin-marked and runtime-owned live children from generic controls', async () => {
    ...
    const edited = await remote.editPrompt(request({
      requestId: 'edit-1' as SessionEditPromptRequest['requestId'],
      sessionId: originChild.id,
      atSeq: 0,
      content: [{ type: 'text' as const, text: 'rewritten' }],
    }))
    expect(edited.ok).toBe(false)
    if (!edited.ok) expect(edited.error.code).toBe('session/agent-busy')
