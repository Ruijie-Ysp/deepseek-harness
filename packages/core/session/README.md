...
`deriveMessages()` caches deep-frozen projections and returns a fresh array per call. The five surface event types (`system/message`, `user/message`, `user/edit`, `assistant/message`, `tool/result`) supply their recorded message identities and content; an empty-content system node projects to no message, and a `user/edit` projects its rewritten message at the replaced position. Plugin-owned projections change derived content without mutating recorded messages. Replacements and projection decisions invalidate the cache. Embedded Assistant streams and `assistant/attempt` events remain replay and diagnostic data only.
...
The model receives the complete messages from `system/message`, `user/message`, `user/edit`, `assistant/message`, and `tool/result` surface entries with logged projections applied, the system prompt first. Identities, roles, sources, and unmodified blocks retain their original values; projections never mint identities. Direct prompts and injected context remain separate `user/message` events whose sources preserve their attribution; an edited message derives from the `user/edit` event's rewritten `message`. Embedded streams, `assistant/attempt`, boundaries, and other log-only facts add no message.
