...
describe('Agent', () => {
  ...
  it('followup with a replacesSeq-marked source logs user/edit shadowing the tail', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('edit'), { provider: 'mock', model: 'mock' })

    send(agent, 'original prompt')
    await agent.whenIdle()
    const userSeq = agent.session.snapshotEvents().find(event => event.type === 'user/message')?.seq
    const assistantSeq = agent.session.snapshotEvents().find(event => event.type === 'assistant/message')?.seq
  ...
  it('fails loud when an edit target is not a current surface node', async () => {
    const adapter = new MockAdapter([textResponse('ok')])
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('edit-miss'), { provider: 'mock', model: 'mock' })
    ...
    const ended = [...agent.session.snapshotEvents()].reverse().find(event => event.type === 'turn/end')
    expect(ended?.type === 'turn/end' && ended.data.reason.kind).toBe('error')
    expect(agent.session.snapshotEvents().some(event => event.type === 'user/edit')).toBe(false)
    expect(agent.status).toBe('idle')
  })
})
