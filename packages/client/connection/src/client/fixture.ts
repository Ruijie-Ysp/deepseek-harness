...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const sessionApi: FixtureSessionApi = {
    ...
    editPrompt: (request) => {
      ...
      const nodes = foldSurface(logOf(id)).nodes
      if (!Number.isSafeInteger(request.atSeq) || request.atSeq < 0
        || !nodes.includes(SessionSeq(request.atSeq))) {
        return sessionErr({
          code: 'session/edit-target-invalid',
          message: `seq ${String(request.atSeq)} is not a current message on the session surface`,
          details: { atSeq: request.atSeq },
        })
      }
      ...
      const userText = request.content.map(block => block.text).join('')
      const turn = nextTurn.get(id) ?? 0
      nextTurn.set(id, turn + 1)
      setRunning(id, true)
      append(id, { type: 'turn/start', data: { turn } })
      append(id, {
        type: 'user/edit',
        surfaceOp: { op: 'replace', start: request.atSeq, end: tail },
        sourceEventSeqs: shadowed,
        data: { message: userMessage([{ type: 'text', text: userText }]), replacesSeq: request.atSeq },
      })
