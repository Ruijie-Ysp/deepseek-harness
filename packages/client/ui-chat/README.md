...
- **Message edit rewrites text only** — a pencil action on a settled text-bearing user bubble (and on a prior edit) enters composer edit mode: the draft adopts the message text, a banner announces the rewrite, and submit routes through `session.editPrompt`, which regenerates the conversation from that point and preserves the message's non-text blocks (images) verbatim ([decision](../../../.agents/notes/implemented/architecture/2026-08-20-user-message-edit-resubmit.md)). Image-only bubbles and steering bubbles offer no edit control, and the edit path admits no new image content.
