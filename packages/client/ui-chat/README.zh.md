...
- **消息编辑只改写文本**——已定稿的带文本 user 气泡（以及之前的编辑）上有铅笔操作，点击进入输入框编辑模式：草稿采纳消息文本，横幅提示改写，提交走 `session.editPrompt`，从该点重新生成对话，并原样保留消息的非文本块（图片）（[决策](../../../.agents/notes/implemented/architecture/2026-08-20-user-message-edit-resubmit.zh.md)）。纯图片气泡与 steering 气泡没有编辑控件，编辑路径也不能新增图片内容。
