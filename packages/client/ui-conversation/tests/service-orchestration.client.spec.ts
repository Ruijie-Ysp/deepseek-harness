...
async function bench(maxConcurrentFileUploads = 2) {
  ...
  await runtime.sessions.add({
    id: 's1',
    session: { prompt, updateQueue, cancel, editPrompt, loadOlder },
  })
  ...
  return { runtime, fiber, root, scoped, hub, shell, prompt, updateQueue, cancel, editPrompt, loadOlder, reference }
}
...
describe('InputHub queue steering (empty-draft accelerated Enter)', () => {
  ...
  it('refuses images in edit mode with a notice and keeps the draft', async () => {
    const b = await bench()
    b.shell.beginEdit(5, 'rewritten prompt')
    b.shell.addAttachments(['img-1'] as never)
    b.shell.submit()
    await vi.waitFor(() => {
      expect(b.shell.notices.getSnapshot()).toEqual(
