...
The four message-producing types (`SurfaceEventType` — `user/message`, `user/edit`, `assistant/message`, `tool/result`) carry surface metadata declaring how they join the ordered derived surface. See the [session surface Agent Note](../../.agents/notes/implemented/architecture/2026-06-18-session-surface.md).
...
/**
 * Rewrite one settled user message after explicitly resuming its Session.
 * @param request - Session identity, the targeted surface seq, and replacement text.
 * @param signal - caller cancellation before the rewrite is admitted.
